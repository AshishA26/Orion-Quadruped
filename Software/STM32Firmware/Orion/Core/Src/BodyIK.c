...
/**
 * Applies body tilt/translation to dynamic foot coordinates and calculates Leg IK.
 * footPositions: Array of 4 vectors [x, y, z] representing each leg's target position.
 * Order: 0=FL, 1=FR, 2=BL, 3=BR
 */
                               ...
                               float roll, float pitch, float yaw) 
{
    ...
    float ix = cy*cp; float iy = cy*sp*sr - sy*cr; float iz = cy*sp*cr + sy*sr;
    float jx = sy*cp; float jy = sy*sp*sr + cr*cy; float jz = sy*sp*cr - cy*sr;
    float kx = -sp;   float ky = cp*sr;            float kz = cp*cr;

    LegIK_t* legs[4] = {&legFrontLeft, &legFrontRight, &legBackLeft, &legBackRight};

    for (int i = 0; i < 4; i++) {
        // Leg Origins relative to Center of Mass
        float comOriginX = legs[i]->IS_FRONT_LEG ? (BODY_LENGTH_MM / 2.0f) : -(BODY_LENGTH_MM / 2.0f);
        float comOriginY = legs[i]->IS_LEFT_LEG  ? (BODY_WIDTH_MM / 2.0f)  : -(BODY_WIDTH_MM / 2.0f);
        float comOriginZ = 0.0f;

        // Extract the raw foot trajectory coordinates generated by the gait function
        float footRelX = footPositions[i][0];
        float footRelY = footPositions[i][1];
        ...
        updateLeg(legs[i], XYZ_1_X, finalY, XYZ_1_Z);
    }
}