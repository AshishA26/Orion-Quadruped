...
RS_VERT = 5
DPAD_VERT = 7 # Pitch (or move pivot point forward/backward when touchpad held)
DPAD_HORZ = 6 # Roll (or move pivot point left/right when touchpad held)
...
BTN_R3 = 11
BTN_PS = 12
BTN_TOUCHPAD = 13  # Hold to control pivot point
...
class STM32Bridge(Node):
    ...
    def joy_callback(self, msg):
        ...
        # --- Pivot Point Control ---
        # If touchpad is held, use the Dpad to adjust the pivot point
        if msg.buttons[BTN_TOUCHPAD] == 1:
        ...
        payload = struct.pack('<B9f', cmd_type, lin_x, lin_y, ang_z, 
        ...
        self.get_logger().info(f"Sent: X:{lin_x:.2f}, Y:{lin_y:.2f}, Z:{ang_z:.2f} \
            ...
            Yaw:{math.degrees(self.yaw):.1f}, Height:{self.z_height:.1f}, CmdType:{cmd_type}, \
